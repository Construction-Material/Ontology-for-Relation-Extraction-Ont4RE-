...
class BagREDataset(data.Dataset):
    ...
    def eval(self, pred_result, threshold=0.5):
        ...
        tpr = []
        fpr = []
        total_negative = len(pred_result) - total
        ...
        for i, item in enumerate(sorted_pred_result):
            ...
            tpr.append(float(correct) / float(total))
            fpr.append(float(i + 1 - correct) / float(total_negative))
        ...
        micro_p = sklearn.metrics.precision_score(label_vec, pred_result_vec, labels=list(range(1, len(self.rel2id))), average='micro', zero_division=0)
        micro_r = sklearn.metrics.recall_score(label_vec, pred_result_vec, labels=list(range(1, len(self.rel2id))), average='micro', zero_division=0)
        micro_f1 = sklearn.metrics.f1_score(label_vec, pred_result_vec, labels=list(range(1, len(self.rel2id))), average='micro', zero_division=0)

        macro_p = sklearn.metrics.precision_score(label_vec, pred_result_vec, labels=list(range(1, len(self.rel2id))), average='macro', zero_division=0)
        macro_r = sklearn.metrics.recall_score(label_vec, pred_result_vec, labels=list(range(1, len(self.rel2id))), average='macro', zero_division=0)
        macro_f1 = sklearn.metrics.f1_score(label_vec, pred_result_vec, labels=list(range(1, len(self.rel2id))), average='macro', zero_division=0)
        
        pred_result_vec = score_vec >= best_threshold
        max_macro_f1 = sklearn.metrics.f1_score(label_vec, pred_result_vec, labels=list(range(1, len(self.rel2id))), average='macro', zero_division=0)
        max_micro_f1_each_relation = {}
        for rel in self.rel2id:
            if rel != 'NA':
                max_micro_f1_each_relation[rel] = sklearn.metrics.f1_score(label_vec, pred_result_vec, labels=[self.rel2id[rel]], average='micro', zero_division=0)

        p_100 = np_prec[99] if len(np_prec) > 99 else np_prec[-1]
        p_200 = np_prec[199] if len(np_prec) > 199 else np_prec[-1]
        p_300 = np_prec[299] if len(np_prec) > 299 else np_prec[-1]

        return {'np_prec': np_prec, 'np_rec': np_rec, 'max_micro_f1': max_micro_f1, 'max_macro_f1': max_macro_f1, 'auc': auc, 'p@100': p_100, 'p@200': p_200, 'p@300': p_300, 'avg_p300': (p_100 + p_200 + p_300) / 3, 'micro_f1': micro_f1, 'macro_f1': macro_f1, 'max_micro_f1_each_relation': max_micro_f1_each_relation, 'np_tpr': np_tpr, 'np_fpr': np_fpr}
