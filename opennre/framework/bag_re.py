...
class BagRE(nn.Module):
    ...
    def train_model(self, metric='auc'):
        best_metric = 0
        # 创建一个空的DataFrame来存储训练过程中的各项指标
        training_records = pd.DataFrame(columns=['Epoch', 'Avg_Loss', 'Avg_Acc', 'Avg_Pos_Acc', 'AUC', 'Micro_F1', 'Macro_F1'])
        ...
        for epoch in range(self.max_epoch):
            ...
            # 在DataFrame中添加一行新的记录
            training_records = training_records.append({'Epoch': epoch,
                                                        'Avg_Loss': round(avg_loss.avg, 4),
                                                        'Avg_Acc': round(avg_acc.avg, 4),
                                                        'Avg_Pos_Acc': round(avg_pos_acc.avg, 4),
                                                        'AUC': round(result['auc'], 4),
                                                        'Micro_F1': round(result['max_micro_f1'], 4),
                                                        'Macro_F1': round(result['macro_f1'], 4)}, ignore_index=True)
