...
parser.add_argument('--result', default='cnn', help='Save result name')
...
parser.add_argument('--max_epoch', default=5, type=int, help='Max number of training epochs')
...
if not os.path.exists('result/' + args.result):
    os.mkdir('result/' + args.result)

# Train the model
if not args.only_test:
    training_records = framework.train_model(args.metric)
    # Save metrics
    # training_records = framework.train_model()
    training_records.to_csv(os.path.join('result/' + args.result, 'training_records.csv'), index=False)
...
# .npy格式
np.save('{}_p.npy'.format('result/cnn/' + args.result), result['np_prec'])
np.save('{}_r.npy'.format('result/cnn/' + args.result), result['np_rec'])
np.save('{}_tpr.npy'.format('result/cnn/' + args.result), result['np_tpr'])
np.save('{}_fpr.npy'.format('result/cnn/' + args.result), result['np_fpr'])
json.dump(result['max_micro_f1_each_relation'], open('{}_mmicrof1_rel.json'.format('result/cnn/' + args.result), 'w'), ensure_ascii=False)